...
class Environment():
    ...
    def step_1(self,action):
        ...
        if (current_train/current_alpha) + current_communication > current_duration:
            return self.get_state(), 0, False, {"vehicle %d cannot be scheduled, because it cannot compelte train and communication in duration time.", current_vehicle.get_id()}, [self.resource_block_decoder(self.current_state[action]),alpha]
        ...
        if resource_pool_update_flag == False:
            return self.get_state(), 0, False, {"Fail to update the resource pool by RB [%d,%d,%d]", selected_resource_block[0], selected_resource_block[1], selected_resource_block[2]},[self.resource_block_decoder(self.current_state[action]),alpha]
    ...
    def step_withour_alpha(self, action):
    ...
    def resource_block_decoder(self, coded_resource_block):
        if coded_resource_block == 0:
            return [0,0,0]
        str_coded_resource_block = str(coded_resource_block)
        bandwith_index = int(str_coded_resource_block[0])
        start_time = int(str_coded_resource_block[5:])
        end_time = int(str_coded_resource_block[1:5])
        if start_time > 1000:
    ...
    def get_RTG_state(self):
        ...
        for item in self.current_state[:-4]:
            if item == 0:
                ...
                RTG_state.append(self.resource_block_decoder(item))
        RTG_state.append(self.current_state[-4])
        RTG_state.append(self.current_state[-3])
        RTG_state.append(self.current_state[-2])
        RTG_state.append(self.current_state[-1])
        return RTG_state
